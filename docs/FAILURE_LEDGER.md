...
- 2026-09-11 05:05 CEST: fresh immutable V6 stage job `dabe0bb8-445f-4150-b88f-43d3cb06b784` failed in WF02 execution `17644`. The first two semantic gateway calls succeeded; storyboard gateway execution `17647` reached `cohere/north-mini-code:free` with the new tool schema but the n8n HTTP node aborted at its configured 50,000 ms bound (`ECONNABORTED timeout of 50000ms exceeded`) before any model response. Do not resume/repair this job. The normalizer also mislabeled the network timeout as `required structured tool call missing`; diagnostics must prefer upstream request errors. Before another job, inspect the source timeout and reduce/measure the storyboard request size/latency rather than blindly inflating timeouts; if the already-proven bounded 70s route remains the documented measured budget, restore that intended bound explicitly and keep the outer caller bound above it.

- 2026-09-11 05:08 CEST: exact diagnostic replay of execution `17647` request body directly against Kilo with the full Cohere function/tool schema also failed to return within 75 seconds. This proves the tool-schema transport materially worsens latency for the current long storyboard request and cannot be treated as a reliable fast path. No product job/runtime mutation occurred. Do not solve this by further timeout inflation. Re-evaluate the long-generation transport/model choice using current free models that support structured output or a bounded non-tool JSON path with strict downstream validation.

- 2026-09-11: After switching the V6 storyboard route from tool-call transport to bounded plain JSON, the targeted regression run correctly passed the new storyboard/plain-JSON checks but failed on stale test `tests/v6_tool_call_structured_transport_regression.mjs`, which still asserted the old generic error text `long structured route requires response_schema`. This is a regression-maintenance mismatch, not a product/runtime failure. No deployment or product job occurred. Update that test to assert tool-call transport only for `final_story`, while storyboard must assert `plain_json`.

- 2026-09-11: Full V6 regression run after the storyboard plain-JSON transport patch exposed another stale regression, `tests/v6_storyboard_evidence_canonicalization_regression.mjs`, whose regex still expected the old `model_request` without the now-required `response_schema`. Product source was unchanged and no runtime/job mutation occurred. Update only the stale assertion so it verifies the current storyboard response schema remains carried to the gateway/WF02 boundary.

- 2026-09-11: Re-running the full V6 suite after updating the storyboard evidence regression exposed a second stale assertion in the same file: it simultaneously required and forbade `response_schema` in the storyboard model request. This contradiction predates the current canonicalization boundary. No runtime/job mutation occurred. Remove only the obsolete negative assertion; the current contract intentionally carries `response_schema` for downstream WF02 validation while the gateway defers storyboard schema enforcement.

- 2026-09-11: Post-deploy webhook readiness loop for the V6 plain-JSON gateway used malformed nested shell/SQL quoting and returned empty rows for 20 checks, even though the prior parameter-safe verification had already seen the stage webhook. This is a read-only operator verification mistake, not evidence that the webhooks disappeared. No runtime/job mutation occurred. Recheck webhook_entity with parameter-safe Python/psql before creating a new job.

- 2026-09-11: Read-only decoder attempt for fresh WF02 execution `17682` incorrectly combined a pipeline into `docker exec -i node -` with a heredoc for the Node program. The heredoc consumed stdin, so the execution payload never reached the decoder (`NO_DATA`, pipeline SIGPIPE 141). No runtime/job mutation occurred. Retry with `node -e` so stdin remains available for the Flatted execution payload.

- 2026-09-11: Stage deployment mistake: importing `n8n/workflows/WF02-plan-script-and-scenes.json` directly used its source workflow ID `TJfA4ZYUEKSTad6k`, so the source workflow received the semantic-route/failure-correlation changes while stage workflow `V6SXn6bN7HHPMdsJ` remained on its older version. Fresh stage job `302d93ba-405f-4aac-a816-817615f23cd6` therefore still called the V6 gateway with route `default`; gateway execution `17689` hit Nvidia overload and WF02 execution `17688` again lost `job_id` in its old failure handler. Do not resume this job. Before another product run, patch/import the existing stage WF02 by preserving stage ID and stage-specific references; verify stage runtime text contains the semantic route and `$items` failure fallback. Also verify the source workflow's active state before any rollback/correction so V5 production is not disturbed.

- 2026-09-11: While correcting stage WF02 targeting, readback of stage workflow `V6SXn6bN7HHPMdsJ` exposed pre-existing stage-isolation drift: its research/discovery HTTP nodes point to `http://media-worker:3001` instead of isolated `media-worker-v6`, and `Start Voiceover Generation` targets production WF03 ID `UHxvCZNqaLb1RKMM` instead of stage WF03 `V6Sbpx10h7QUc0iM`. Do not create another V6 stage job until these stage-only mappings are restored. This is a staging wiring defect; production V5 itself is unchanged. Preserve source workflow `TJfA4ZYUEKSTad6k` inactive and patch only stage WF02 ID in place.

- 2026-09-11: Fresh immutable V6 stage job `6a4553e7-8a64-4d30-8ff5-9625a29172c7` failed in WF02 execution `17691` at semantic intake with `semantic candidate model returned invalid JSON [line 1]`. Stage isolation and failure correlation were corrected before this run: the DB row was correctly persisted as `failed|script` with the error, proving the `$items` failure-correlation fallback now works in runtime. Do not resume this job. Inspect paired gateway execution `17692` before changing the semantic route; correction must be based on the exact free-model response, not another timeout increase or Gemini fallback.

- 2026-09-11: Read-only WF03 inspection script printed the needed `Rewrite Narration For Exact Duration` request (`={{ $json.model_request }}`, timeout 180000) but then hit a local Python `NameError` because the staged script accidentally contained a trailing `PY` token. No runtime/job mutation occurred. Do not treat the non-zero exit as a workflow defect; rerun only if more WF03 fields are needed.

- 2026-09-11: Fresh immutable V6 stage job `8bd618dd-bf7e-4129-80d2-5e79b813f730` progressed through semantic intake/resolution and reached storyboard validation, then failed in WF02 execution `17701` at diagram shot `S3A`: the free storyboard model emitted relation kinds `produces` and `contributes`, while the high-level diagram contract currently accepts only `flow|joins|splits|emits|blocks|contains|transforms`. This is a deterministic alias-boundary gap, not a reason to weaken diagram grounding. The job is terminal `failed|script` and must not be resumed. Systemic correction: canonicalize `produces -> transforms` and `contributes -> joins` before strict validation/compilation, add an exact regression, then create a new job.

- 2026-09-11: Engineering edit mistake while implementing the proven `produces/contributes` relation aliases: the scripted patch updated WF02 successfully but then stopped because it assumed an obsolete compiler source anchor; `diagram-compiler.mjs` already uses a `RELATION_KIND_ALIASES` map. No runtime deployment or product retry occurred. Working tree is intentionally partial. Correction: extend the existing compiler alias map instead of replacing the old inline validator expression, add the regression, then test before commit/deploy.

- 2026-09-11: V6 relation-alias deploy encountered host disk exhaustion during n8n WF02 import: PostgreSQL returned `could not extend file ... No space left on device`. The new stage worker image had already been built and started healthy; no product retry occurred. SentinelX could not even stage the first ledger-write script because `/` was at 100%. A bounded recovery removed only Docker build cache with `docker builder prune -af` (no volumes, no running containers/images), reclaiming 3.611 GB and restoring about 2.0 GB free. Before retrying import, verify stage WF02 active/current state because import had begun by deactivating it.

- 2026-09-11: Fresh immutable V6 stage job `f68e698f-c7d3-4c66-9853-9a2f3ce328ea` passed semantic intake/resolution and reached the storyboard director, then failed in WF02 execution `17740` because gateway execution `17743` timed out the free `cohere/north-mini-code:free` storyboard call at the explicit 50-second internal bound. The gateway returned `provider_exhausted=true`; the job correctly persisted terminal `failed|script`. Do not resume/retry this job. Before another product run, benchmark this exact storyboard prompt against the currently available free long-generation candidates with bounded timing and choose a role-specific route based on measured completion, not another arbitrary timeout increase.

- 2026-09-11: Fresh immutable V6 stage job `a9b08920-d81a-4fe9-a485-be73089ede38` failed at the first semantic call in WF02 execution `17749`. Gateway execution `17750` routed to `liquid/lfm-2.5-2.6b:free`, but Kilo returned an upstream throttling/batching error: `Try spacing your requests out using the batching settings under 'Options'`; no model text was produced, so downstream parsing correctly failed closed and the job persisted terminal `failed|script`. Do not resume this job. This proves a single free semantic provider is not reliable enough; before another run, benchmark the exact semantic prompt on other current free models and implement one bounded free-provider failover with no sleep/retry loop and no paid/Gemini fallback.

- 2026-09-11: First regression run after adding the bounded semantic free-provider failover exposed a stale gateway topology assertion: `tests/v6_free_only_model_gateway_regression.mjs` still required `Normalize Free Model Response -> Respond`, while the intentional new path is `Normalize Free Model Response -> Use Semantic Fallback?`, with the false branch going to Respond and the true branch making exactly one Cohere free fallback attempt. No runtime deployment or product job occurred. Update only the stale topology assertion, then rerun the full V6 suite.
...
- 2026-09-11: Fresh V6 job `056c7b04-3091-48bf-874f-1f35b2cdb051` passed semantic and evidence-grounded resolution on Liquid after migration 027, then immutable-failed at storyboard because `poolside/laguna-s-2.1:free` exceeded the gateway 65s bound. This is provider latency on the exact production storyboard request; do not retry this job or inflate timeout. Benchmark alternate currently-free text models on the exact failed prompt and add a bounded storyboard failover only if one proves the contract.

- 2026-09-11: Fresh V6 job `d2225831-6082-43a1-aa62-75958446886d` reached the Ling storyboard route successfully (5869 chars, valid JSON) but immutable-failed validation because Ling emitted scene IDs `SC1`, `SC2`, ... while the durable contract requires `S1`, `S2`, ... . Shots already used canonical IDs (`S1A`, etc.). This is a systematic schema-label alias, not a content failure. Add bounded canonicalization `SC<n> -> S<n>` at the storyboard validation boundary and keep strict sequential validation afterward.

- 2026-09-11: Fresh V6 job `2f0bfb45-21b5-4d4d-808a-4aeee4bd24e0` passed Ling scene-ID canonicalization, then immutable-failed on diagram `S3B`: scene evidence was `[S4,S6,S7]` while the diagram correctly cited the narrower subset `[S6,S7]`. Validator incorrectly required diagram evidence to equal the whole scene evidence set. Root cause is an over-strict grounding equality check; diagram evidence must be a non-empty subset of its scene evidence, never an unrelated superset.
- 2026-09-11: WF02 stage deploy verification used an incorrectly quoted PostgreSQL readiness query, so `"workflowId"` was folded to `workflowid` and the verification loop falsely reported that n8n was not ready after a successful import/publish/restart. Root cause: operator quoting error in the verification script, not a workflow/runtime failure. Correction: verify readiness with a quoting-safe query and inspect the published WF02 export before any new job.
- 2026-09-11: Fresh V6 E2E submit attempt used Python `urllib` against the public stage webhook and received HTTP 403 before any job was created. Root cause is operator transport choice at the public edge (the same endpoint had previously worked with curl), not a product workflow failure. Correction: use the already-proven curl submit path or an internal n8n-local webhook invocation; no product job exists from this attempt.
- 2026-09-11: Fresh immutable V6 job `4784eb3f-d636-4393-98c2-3acee8335584` passed semantic/resolution and returned a grounded Ling storyboard, but WF02 rejected diagram shot `S2A` with `diagram shot S2A entity is invalid`. Exact execution inspection showed the only invalid entity field was `lane:"top"` on a `split` diagram input; the compiler only accepts `left|right|center`, and split layout ignores lane entirely. Root cause: bounded model vocabulary drift at the storyboard canonicalization boundary. Correction must deterministically canonicalize vertical lane aliases (`top|bottom|middle`) to the supported neutral `center`/lane contract before validation, with matching compiler regression; do not retry this job.
- 2026-09-11: A diagnostic exact-storyboard compiler probe invoked `docker exec ... node -` / `docker run ... node -` without `-i`, so the heredoc was not delivered to Node and the command produced no diagnostic output despite exit 0. Root cause: operator stdin wiring mistake. Correction: rerun the read-only probe with `-i`; no product/runtime state changed.
- 2026-09-11: Fresh immutable V6 job `bce87a29-d33f-40ff-91e4-ad41a1aeed64` failed at `Validate Storyboard` with `storyboard shot S2A is invalid`. Exact output had six `must_show` items while the contract permits five. Read-only downstream probing also proved the same `S2A` would next fail the diagram compiler because a `split` graph contained three `input` roles (`Sunlight`, `Blue light`, `Red light`) instead of one root input; the model correctly encoded the latter two as children of the root via incoming `splits` edges. Root cause is bounded structured-output vocabulary/cardinality drift at the canonicalization boundary, not bad research. Correction: bound ordered must-show/must-not-show lists to contract maxima and deterministically normalize `split` graph roles from topology (single indegree-zero root input; child inputs -> process; terminal subject -> result), mirrored in WF02 and the diagram compiler. Do not retry this job.
- 2026-09-11: Fresh immutable V6 job `5971e3e4-79c3-45e3-994c-318f06209d47` passed storyboard planning, persisted `storyboard-first-v1`, generated a natural continuous Edge voiceover (15.945333s), and built exact story-unit timings, but WF03 `Persist Voiceover Result` returned `job_updated=false`. Exact deployed/source inspection showed the SQL lock/ok predicates still whitelist only `inventory-first-story-v1` and `visual-facts-story-v1`; the same stale two-version predicate also exists in `Persist Duration Rewrite`. Root cause: WF03 SQL contract drift after adding storyboard-first support in JS. Correction: admit `storyboard-first-v1` in every WF03 story-version SQL predicate, regression-test the whole workflow, then deploy stage WF03 and run a new job. Do not resume this failed job.
- 2026-09-11: Proactive V6 SQL contract scan after the WF03 fix found two additional stale storyboard-version gates before they caused another product job: WF04 `Begin Visual Stage` still allows only `inventory-first-story-v1|visual-facts-story-v1`, and WF05 `Persist Render Success` / existing-success branch likewise has no `storyboard-first-v1 -> storyboard-first-render-v1` case. Root cause: storyboard-first support was added to JS/runtime validation without updating all downstream SQL transition/persistence predicates. Correction: patch both workflows together with regressions before the next E2E.
- 2026-09-11: Immutable V6 E2E `595f9221-3dda-4f6f-b128-d2cd8e0dfe33` failed in WF02 with `diagram shot S3A grounding/spec is invalid`. Exact Ling storyboard carried valid outer scene/shot grounding (`scene S3` with `[S1,S2]`, shot `S3A`) but omitted duplicated `diagram_spec.shot_id` and `diagram_spec.grounded_fact_ids`. Root cause is model-boundary duplication drift: validator requires inner copies even when the parent shot already provides authoritative identity/grounding. Correction must canonicalize missing inner diagram identity/grounding from the already-validated parent shot/scene before strict diagram validation; do not relax evidence validity or invent new facts.
- 2026-09-11: Immutable V6 E2E `9d52d086-e51f-4775-8253-d0fc27ddadc5` failed in WF02 with `semantic discovery returned fewer than two usable results`. Exact execution `18032` shows both candidate searches reached SearXNG successfully at transport level but each returned `number_of_results: 0` / `results: []` for ordinary English queries (`Why does the sky appear blue scientifically?`, `symbolism of blue sky in human culture and art`). This is a search-provider availability/result defect, not storyboard/model validation. Diagnose SearXNG directly before any retry; do not weaken evidence requirements or reuse the failed job.
- 2026-09-11: Immutable V6 E2E `e3d70c30-9933-44ac-9755-695691cd7183` passed WF02, persisted `storyboard-first-v1`, completed Edge TTS at 16.033s, entered WF04, built constructed diagram `S2A`, stored/fingerprinted it, then failed persisting the media-library row. Exact cause: `Verify Reserved Visual Identity` rebuilt output from `Expand Reserved Story Assets` plus fingerprint only, dropping `visual_path` and stored media metadata produced by `Validate Stored Reserved Visual`; `Persist Reserved Visual` therefore sent NULL `local_path`, violating `media_library_assets.local_path NOT NULL`. Fix must merge the validated stored-media payload into identity verification output before DB persistence; failed job remains immutable.
- 2026-09-11 operator mistake: WF04 stage backup export attempted to reuse `/tmp/V6SptgRPF8bcTznj.json`, which was not writable by the n8n container user; export failed with EACCES before import/publish/runtime mutation. Retry must use a unique container temp path owned by n8n, not the stale filename.
- 2026-09-11: Immutable V6 E2E `9ff33a97-55b1-451a-a995-e42d70b305a9` failed in WF02 with `storyboard shot S1A is invalid`. Exact Ling output used the already-known scene alias form `SC1`, and consistently emitted its child shot as `SC1A`; scene canonicalization converted only `SC1 -> S1`, leaving the shot identity unnormalized. Root cause is incomplete identity canonicalization at the model boundary. Correction must canonicalize `SC<n><letter> -> S<n><letter>` for outer shot IDs and diagram inner shot IDs before the existing strict sequential identity check.
- 2026-09-11 operator note: recent-job status probe used invalid psql shell escaping for empty-string literals and failed read-only. No product state changed. Use argv-safe psql invocation for the retry.
- 2026-09-11 operator note: argv-safe recent-job probe incorrectly sent SQL to the intermediate shell stdin, producing empty diagnostics. No product state changed. Retry uses container env lookup plus direct psql `-c` argv.
- 2026-09-11: Fresh immutable V6 job `b4bc86d3-d1cd-42a3-9548-835b433d2f81` reached WF03 voiceover and failed at the bounded duration-story rewrite with `duration story rewrite model unavailable or invalid after bounded gateway recovery`. Exact gateway response is pending inspection; do not resume this job.
- 2026-09-11 operator note: exact duration-rewrite replay attempted to call the internal V6 gateway with `curl` inside the n8n container, but that image has no curl binary. No product state changed. Retry uses Node fetch in the same container.
- 2026-09-11 duration-rewrite model check: exact failed-job rewrite prompt routed through the proven Ling storyboard path returned `finish_reason=stop` and valid JSON, but the JSON contained zero `tokens` instead of the required exact token array. Ling plain-JSON storyboard transport is therefore not sufficient for the duration-rewrite contract; no product state changed.
- 2026-09-11 duration-rewrite model check: the exact failed-job rewrite request routed through Liquid with the 16k final-story budget returned `finish_reason=stop`, but the payload omitted required `tokens`; gateway schema validation correctly rejected it. The current exact-token rewrite contract is not reliably followed by the proven free models and should be simplified rather than adding timeouts/retries. No product state changed.
- 2026-09-11 operator note: a read-only PostgreSQL probe for the current WF03 duration-rewrite failure did not execute because nested shell quoting broke before docker/psql invocation. No product/runtime state changed. Future probes must avoid nested SQL shell quoting and pass SQL through a temporary argv-safe script or stdin.
- 2026-09-11 operator note: corrective push to the documented direct SSH repository URL failed with publickey because the existing deploy key is not selected by the default root SSH invocation. No repository config or credentials were changed. Resolve the existing key path/config without printing key material, then push the pending docs commits through that key.
- 2026-09-11 operator note: fallback check for root GitHub CLI authentication could not run because gh is not installed in the root sudo PATH on this host. No repository/runtime state changed. Do not rely on gh for VPS push transport.
- 2026-09-11 operator note: argv-safe read-only PostgreSQL probe for job b4bc86d3-d1cd-42a3-9548-835b433d2f81 reached Docker but failed because the SentinelX service account lacks direct access to /var/run/docker.sock. No SQL or product/runtime mutation occurred. Future container probes must use sudo docker, matching the privileged repository/runtime path.
- 2026-09-11 operator note: one mandatory-read call was rejected client-side before host execution because SentinelX `sentinel_exec.command` was supplied as an argv array instead of the required string. No server/product state changed. Use the documented string command shape.
- 2026-09-11 operator note: a bounded read-only decoder request for n8n execution 18056 was blocked by OpenAI tool safety before host execution. No server/database/product state changed. Do not bypass the tool guard; continue from already-preserved exact duration-rewrite evidence and use ordinary bounded runtime/source inspection that does not request raw execution payloads.

- 2026-09-11: WF03 unit-level duration rewrite targeted regression reached three new PASS results, then stopped on stale `wf03_duration_bracket_controller_regression.mjs` wording that still required the retired literal `one flat tokens array`. The controller values themselves remained correct. This is regression-contract drift caused by intentionally simplifying model transport from per-word token objects to unit narration JSON; runtime was not deployed. Update only the stale wording assertion, then rerun the unchanged controller and full V6/WF03 gate.

- 2026-09-11: Live V6 component probe of the simplified WF03 unit-level duration-rewrite schema used route `final_story` with three grounded Russian units and an exact 36-word instruction. The internal gateway returned HTTP 200 but `provider=none`, `provider_exhausted=true`, no units, and total words 0 after about 12 seconds. This is a current free-provider route availability failure before unit-schema validation, not evidence against the unit-level rewrite contract. Do not add sleeps/timeouts; next bounded proof uses the already-proven Ling storyboard route with the same unit schema.

- 2026-09-11: After live Ling proved the simplified unit-level duration rewrite at exactly 36 words, WF03 source was switched from `final_story` to the proven `storyboard` route. The targeted rerun then passed the unit-schema and recovery regressions but stopped on one stale assertion in `wf03_live_sentence_completion_regression.mjs` that still expected route `final_story`; actual source correctly reported `storyboard`. This is test-contract drift only; stage runtime was not deployed. Update that single route expectation, then rerun the unchanged tests.

- 2026-09-11: A deliberately over-broad `tests/*.mjs` sweep (excluding only the real-provider dry run) stopped immediately at legacy `editorial_shot_plan_contract_regression.mjs` because it dereferenced retired node `Build Final Inventory Story`. The current V6 storyboard-first workflow intentionally no longer contains that inventory-first node. This is legacy regression-scope drift, not evidence that the current WF03 unit-level duration rewrite failed. The targeted current WF03/V6 regression set had already passed 6/6. Do not restore retired inventory-first nodes merely to satisfy this legacy test; use the current V6/WF03 gate for this change and migrate legacy coverage separately if required.

- 2026-09-11: Read-only monitor for fresh V6 job `e863d86f-faf8-43b3-aec8-7f688a4843ae` used nonexistent `public.jobs.output_video_path`; PostgreSQL rejected the query before any product state was read or changed. This is an operator schema-assumption failure, not product evidence. Monitoring must use verified columns only.

- 2026-09-11: Fresh immutable V6 job `e863d86f-faf8-43b3-aec8-7f688a4843ae` (`Почему небо голубое?`, ru, 15s) passed WF02 and the corrected WF03 duration-rewrite path, persisted voiceover duration 16.270s, then failed in stage WF04 execution `18120` with `Reserved visual persistence cardinality 1/4 [line 1]`. This is a new visuals-stage persistence/cardinality defect; the job is immutable and must not be resumed. Inspect exact reserved visual rows and WF04 persistence logic before changing code; do not weaken the cardinality gate or manually supply assets.
