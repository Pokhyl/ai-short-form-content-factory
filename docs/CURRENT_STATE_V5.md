...
### 2026-09-10 V6 storyboard-first stage deployment — verified before fresh job
GitHub branch `rebuild/storyboard-editor-v6-20260910` now contains the storyboard-first path through commit `99ca52add3459badd6615c0f961f0f7b671a3b72`. The isolated stage media worker was replaced with exact image `sha256:fff712a41b0a025f34f7de9f263ed0e1dba77dac9d35ec03ce1b1a1a59a2cf6f`; source/runtime SHA parity passed for server, render-v6, diagram compiler/preview and Remotion components, worker health is `ok`, and SearXNG is reachable. Six isolated V6 workflows are published/current: stage WF01-WF05 plus `V6ModelGatewayFreeOnly`. Both `v6-stage/jobs` and `v6-model-gateway` webhook rows are registered, all six have current version equal to activeVersionId, exact current nodes/connections/settings parity passed against the generated stage import, active executions are zero, and production WF01-WF05 version snapshot is unchanged. Rollback/evidence directory: `/opt/ai-short-form-content-factory-runtime-backups/v6-storyboard-first-20260910T195428Z`. The next product proof must be a completely NEW stage job; earlier failed stage jobs remain immutable.
...
V6 storyboard-first stage proof update, 2026-09-10: fresh job `9bbe6a4b-70dc-46ef-98e6-d06fd15fdfb3` is immutable after WF02 execution `17496` failed. Gateway execution `17497` proves the V6-only StepFun route is not usable even for text-only semantic intake: `finish_reason=length`, zero text, `provider_exhausted=true`. The new WF02 failure handler also failed to recover `job_id`, leaving the row at `created/intake`; this is a systemic handler defect in the rewritten node path. No new product job is allowed until the free text model route and failure persistence are corrected and verified. Production V5 remains unchanged.

V6 free text provider diagnostic, 2026-09-10: live Kilo catalog reported 22 zero-cost/free entries. On the exact semantic-intake contract, `nex-agi/nex-n2.5-pro:free` and `nvidia/nemotron-3-super-120b-a12b:free` produced terminal valid JSON; Nex Mini returned `{}`, while Dots3 Note Preview and LFM2.5 2.6B exhausted output with zero content. V6 correction is therefore constrained to the two passing specific `:free` models; no dynamic paid/Google/Gemini route is authorized.

V6 deployment update, 2026-09-10: first attempt to import the corrected free-text gateway was rejected by n8n as structurally invalid due duplicate Nex node IDs/names created by the mutation script. The import stopped before mutation/publish/restart. Stage and production runtime remained unchanged. Correction must remove the duplicate nodes in source and add uniqueness assertions before another deployment attempt.

V6 live free-gateway probe, 2026-09-10: execution `17501` returned valid JSON through `nvidia/nemotron-3-super-120b-a12b:free`, but only after Nex Pro timed out at the existing 90s bound. The caller itself timed out at 110s. The next correction is provider ordering, not a larger timeout: Nemotron becomes primary; any secondary free attempt remains bounded. No product job was created by this probe.

V6 free-text route is now live in isolated staging from GitHub commit `12255cf9ec26318a9f3618cde92fe3df8c061278`. `V6ModelGatewayFreeOnly` active/current version `c621ae65-9719-4f58-a075-abcffb24cb94` uses one explicit zero-cost model, `nvidia/nemotron-3-super-120b-a12b:free`, with a 45s fail-closed HTTP bound and no Gemini/Google/OpenAI/StepFun/Nex route. A live webhook semantic-intake fixture returned HTTP 200, `provider=kilo-nemotron`, `provider_exhausted=false`, `finish_reason=stop`, and schema-valid question JSON. Stage WF02 already includes the static failure-context recovery correction. Production V5 remains unchanged. Next proof is one completely new stage product job; failed `9bbe6a4b...` remains immutable.

Diagnostic note: direct `require('flatted')` from an ad-hoc `node -e` inside the n8n container is unavailable from that module resolution path. No runtime mutation occurred; execution-data inspection must use a local bounded decoder rather than modifying the container.

V6 stage product proof `9a90fc06-efdb-4e49-b92d-e9f86d3b1f75` is failed/immutable after WF02 execution `17504`. Exact execution-data decode shows the storyboard request reached `V6ModelGatewayFreeOnly`, but gateway execution `17507` timed out at the 45s Nemotron bound and returned provider exhaustion. `Validate Storyboard` correctly failed closed. The centralized failure handler again failed to recover job identity from cross-node references in the task-runner error path, leaving the database row at `created/intake`; that handler design is therefore disproven and must be replaced, not patched with more `$()` variants.

V6 exact storyboard free-model proof, 2026-09-10: on the actual 5,550-character failed-job storyboard prompt, Poolside Laguna S 2.1 free was the only tested model to complete within the 70s diagnostic window, producing valid 5-scene JSON in 56.78s. Nemotron 3.5 Lightning, Nex N2.5 Pro, and Nemotron 3 Super all timed out at 70s on that same prompt. V6 should therefore route storyboard generation specifically to Poolside while retaining the already-proven Nemotron route for shorter semantic contracts; this is role-specific free routing, not a global timeout increase.
...
2026-09-10 22:52 CEST — Fresh V6 storyboard-first stage evidence: job `17970103-3990-44e4-8be6-6296cd57edee` is immutable after WF02 execution `17515`. Poolside route reached the new storyboard director but its otherwise JSON output exceeded `grounded_fact_ids maxItems=3` for scene 2, so gateway execution `17518` failed closed at schema validation. This proves the routed free model is responsive but raw model cardinality cannot be treated as already canonical. Preserve the 1-3 evidence contract; add deterministic compiler normalization at the AI→storyboard boundary, then validate. Failure-handler `job_id` loss remains a separate systemic defect.

2026-09-10 22:58 CEST — Fresh storyboard-first stage job `0b9c8185-ab46-4ae7-b7d2-97571c95181b` is immutable after WF02 `17520`. The Poolside storyboard itself arrived as JSON; validation failed only because diagram entity `S3A:molecules` used neutral `lane:"center"` although the compiler contract accepts only left/right/unset. Add deterministic neutral-lane canonicalization at the AI→diagram compiler boundary; retain fail-closed rejection for other unsupported lane values.

2026-09-10 23:02 CEST — V6 comparison-center implementation note: source edits for `lane:center` were written, but the script stopped before adding the regression because it targeted a non-existent test filename. Runtime is unchanged. Inspect the actual test inventory and continue from the partial working tree; do not reapply the source edits blindly.
...
- 2026-09-10: Fresh V6 job `a1bbfb7c-0251-4f66-9dce-684811cf0549` is immutable after WF02 `17562`. Cohere produced the full storyboard, but its shape was only semantically equivalent to the contract: scalar must_show/must_not_show plus common role/shape/relation aliases and an implicit viewer endpoint. Implement one generic deterministic AI→storyboard/diagram canonicalizer for these evidenced variants, then keep strict validation after normalization.

- 2026-09-10: Fresh V6 job `df45018a-13ec-497e-be8c-808bfbcf2854` is immutable after WF02 `17567`. Cohere long generation stopped normally but emitted one structurally malformed JSON object (parse failure at char 4061). Do not repair braces heuristically. Kilo's current API documents response_format plus tool calling/function JSON Schema; next correction should enforce the existing storyboard contract at transport level if the free Cohere model advertises/supports it.

2026-09-11 04:45 CEST — V6 tool-call transport deploy verification stopped after successful gateway/WF02 import+publish because n8n localhost health was still refusing connections during the bounded wait and `webhook_entity` contained the stage intake webhook but not `v6-model-gateway`. No new product job may be created until bounded n8n startup/log/webhook inspection explains the missing registration. Source commit `96a80a7d985cfa1e114628faf8fe53d8b0071c20` contains the tool-call structured transport change.

2026-09-11 04:54 CEST — The first stage submit/poll after tool-call transport deploy lost its console result because the combined 480-second operator poll hit the SentinelX call timeout. Treat job state as unknown, not failed. Recover the existing latest V6 job and execution state read-only before any new submission; switch to short bounded probes.

2026-09-11 04:57 CEST — Execution `17631` remains to be decoded. The first read-only decoder attempt failed only on file permissions because chmod ran as the n8n container's non-root user. No product mutation occurred; decode as container root or via stdin before any correction/new job.

2026-09-11 04:59 CEST — First real V6 run after tool-call transport, job `e082ebd5-ad1d-4565-ba92-bd21f86dee78`, proved Cohere structured transport works: gateway returned a successful required tool call instead of malformed JSON. WF02 `17631` then failed closed on an under-specified nested diagram contract (`role:center` plus relation target `observer` absent from entities). Strengthen the storyboard function schema for diagram entities/relations and require declared relation endpoints; do not weaken downstream diagram validation and do not reuse this job.

2026-09-11 05:01 CEST — Job `571e00f6-d1d8-4019-b96d-9abf53aad929` proved Kilo/Cohere may ignore forced `tool_choice`: execution `17639` returned `finish_reason=stop`, zero tool calls, but valid 5-scene JSON content. Gateway currently rejects that response before schema validation. Correct the gateway to use tool-call-first plus strict schema-validated content fallback; do not accept malformed JSON and do not retry this job.

2026-09-11 05:04 CEST — Exact replay of `17639` proved the content fallback itself works but exposed schema placement: gateway rejected a canonicalizable storyboard cardinality (`grounded_fact_ids > 3`) before WF02's canonicalizer. Split long routes so storyboard transport defers strict canonical validation to `Validate Storyboard`, while final-story structured output remains schema-validated at the gateway.

2026-09-11 05:05 CEST — Job `dabe0bb8-445f-4150-b88f-43d3cb06b784` is immutable after WF02 `17644`; gateway `17647` timed out at the inner HTTP node's 50s bound before Cohere returned a storyboard. This is now a measured runtime-bound issue, not malformed JSON or visual review. Inspect source/runtime timeout drift and request size before changing it; fix error reporting so network errors are not mislabeled as missing tool calls.
...
2026-09-11 — V6 relation-alias implementation note: WF02 source received the intended `produces -> transforms` / `contributes -> joins` normalization, but the same edit script stopped before compiler/test changes because the compiler had already been refactored to `RELATION_KIND_ALIASES`. Runtime remains unchanged. Continue from the partial working tree by extending that existing alias map, not by reapplying the WF02 edit.

2026-09-11 — Disk recovery note: relation-alias worker image is healthy, but WF02 import failed when PostgreSQL hit host ENOSPC. Root reached 100%. Recovery was limited to `docker builder prune -af`, reclaiming 3.611 GB; no volumes were pruned and production containers/images were left intact. Next action is state verification and only then a clean WF02 stage import/publish.

2026-09-11 — V6 fresh stage job `f68e698f-c7d3-4c66-9853-9a2f3ce328ea` is immutable after WF02 `17740`. Semantic stages passed; exact failure is storyboard gateway execution `17743`, where `cohere/north-mini-code:free` hit the 50s bound under the new plain-JSON transport. Failure correlation/persistence now works. Next proof must benchmark the exact saved storyboard prompt and select a measured free route before creating another job.
...
2026-09-11 infrastructure checkpoint: the portrait-discovery image build filled `/` to 100% during Docker unpack. Emergency recovery (required because even SentinelX could not stage a ledger script) killed only the stuck build processes and ran `docker builder prune -af`; no volumes or product data were touched. Root recovered to ~1.2 GB free. Production and the existing V6 stage runtime were not replaced. A safer stage image replacement must avoid keeping both full 4.55 GB stage images unpacked concurrently.
...
- 2026-09-11: Operator verification mistake after deploying the diagram-evidence-subset WF02 patch: a malformed PostgreSQL readiness query referenced `workflowid` instead of quoted `"workflowId"`, producing a false `n8n did not become ready` result. Import/publish/restart had already completed. Next action is quoting-safe runtime verification only; do not redeploy blindly.
- 2026-09-11: Operator submit transport mistake after WF02 diagram-subset deployment: Python `urllib` to the public `v6-stage/jobs` endpoint returned HTTP 403 before job creation. Runtime verification was green. Retry must use the previously proven curl/internal transport; do not change product logic for this edge-client failure.
- 2026-09-11: Immutable E2E `4784eb3f-d636-4393-98c2-3acee8335584` failed at WF02 diagram entity validation. Exact payload: `S2A` was a valid grounded `split` except model emitted entity lane `top`; supported high-level lanes are `left|right|center`, and split renderer layout does not use lane. This is a canonicalization-boundary defect, not a content/research failure. Next correction: canonicalize `top|bottom|middle` to supported neutral lane consistently in WF02 and diagram compiler, then exact-replay and run a new job.
- 2026-09-11: Read-only diagnostic probe mistake: Docker Node heredocs were run without `-i`, yielding no actual compiler probe. No product state changed; rerun with stdin attached before correcting the current storyboard failure.
- 2026-09-11: Immutable E2E `bce87a29-d33f-40ff-91e4-ad41a1aeed64` exposed two canonicalization defects in one exact storyboard: `S2A.must_show` contained 6 ordered items vs contract max 5, and the grounded split diagram used three input roles even though two were graph children of the root sunlight node. A read-only compiler probe confirmed `split requires one input and at least two outputs/results` would be the next failure. Correct the boundary systemically by bounded list truncation and topology-based split-role normalization in both WF02 and diagram compiler.
- 2026-09-11: Immutable E2E `5971e3e4-79c3-45e3-994c-318f06209d47` reached WF03 successfully: Edge TTS completed at 15.945333s and exact provider word timing/story-unit timing was built. Persistence failed only because WF03 SQL still admits two old story versions and excludes `storyboard-first-v1`; the stale predicate is present in both voiceover persistence and duration-rewrite persistence. Fix all WF03 SQL version gates together before the next E2E.
- 2026-09-11: Preflight scan found downstream SQL drift before retry: WF04 cannot transition a storyboard-first job into visuals, and WF05 cannot persist/reuse storyboard-first render success. These are source defects independent of the last failed job. Patch both together before a new E2E to avoid serial rediscovery.
- 2026-09-11: Fresh immutable V6 job `595f9221-3dda-4f6f-b128-d2cd8e0dfe33` reached Ling storyboard generation, then WF02 rejected `S3A` only because its `diagram_spec` omitted redundant inner `shot_id`/`grounded_fact_ids` while the outer validated shot/scene carried `S3A` and `[S1,S2]`. Next correction is deterministic boundary inheritance of those two fields from parent context, followed by exact replay and a new job.
- 2026-09-11: Fresh immutable V6 job `9d52d086-e51f-4775-8253-d0fc27ddadc5` failed before resolution because SearXNG returned zero results for both ordinary English semantic discovery queries. Next action is direct provider diagnosis/recovery; evidence minimum remains unchanged.
- 2026-09-11: Fresh immutable V6 job `e3d70c30-9933-44ac-9755-695691cd7183` reached WF04 after successful storyboard + 16.033s Edge narration. Constructed diagram generation itself succeeded; persistence failed only because WF04 identity verification dropped `visual_path`/stored-media fields before `media_library_assets` insert. Next correction is data-flow preservation in `Verify Reserved Visual Identity`, not a schema change.
- 2026-09-11 operator note: WF04 stage deploy did not start because backup export hit EACCES on a stale `/tmp/V6SptgRPF8bcTznj.json`; no runtime change occurred. Use a unique export path on retry.
- 2026-09-11: Fresh immutable V6 job `9ff33a97-55b1-451a-a995-e42d70b305a9` failed only because Ling emitted `SC1A` under aliased scene `SC1`; scene aliasing existed but child shot aliasing did not. Next correction is bounded `SC<n><letter> -> S<n><letter>` canonicalization before strict storyboard identity validation.
...
- 2026-09-11 operator note: read-only inspection of job b4bc86d3-d1cd-42a3-9548-835b433d2f81 failed before execution due nested shell quoting around docker/psql. No state changed; use an argv-safe probe next.
- 2026-09-11 operator note: direct SSH push of pending checkpoint commits failed with publickey under the default root SSH selection. No state outside local Git changed; locate and use the pre-existing deploy key explicitly without credential/config mutation.
...
- 2026-09-11: WF03 duration-rewrite transport simplification is now proven by the current targeted gate: 6/6 PASS (`wf03_duration_token_schema`, `wf03_duration_rewrite_recovery`, `wf03_live_sentence_completion`, `wf03_duration_bracket_controller`, `wf03_planned_budget_semantic_floor`, `v6_storyboard_first_voiceover`). The live Ling probe already produced exact 36 total words across U1/U2/U3, and the stale `final_story` assertion has been corrected to the proven `storyboard` route. A deliberately broad legacy `tests/*.mjs` sweep then stopped at retired inventory-first node `Build Final Inventory Story`; that is regression-scope drift, not a current V6/WF03 failure. Do not restore retired nodes. Before source deployment, preserve this failure checkpoint on GitHub, then run the current V6/WF03-scoped deterministic gate plus JSON/diff checks.

- 2026-09-11: Read-only monitor for fresh V6 job `e863d86f-faf8-43b3-aec8-7f688a4843ae` used nonexistent `public.jobs.output_video_path` and PostgreSQL rejected the query before any product state was read or changed. This is an operator schema-assumption failure only. Future monitoring must query only verified jobs columns (or inspect schema first); do not infer product failure from this probe.

- 2026-09-11: Fresh immutable V6 job `e863d86f-faf8-43b3-aec8-7f688a4843ae` (`Почему небо голубое?`, ru, 15s) proved the WF03 duration-rewrite fix past the previous blocker: WF02 completed, WF03 produced/persisted a natural voiceover of 16.270s, then stage WF04 execution `18120` failed closed with `Reserved visual persistence cardinality 1/4 [line 1]`. The job is terminal `failed|visuals` and must not be resumed/repaired. Next action is to inspect the exact reserved-visual persistence state/source and correct the systemic WF04 cardinality defect without weakening gates or manually selecting media.
