...
## 2026-09-05 — multimodal whole-video request cap root cause

After deploying exact-beat visual discovery, the fresh matrix identified a new general WF04 boundary defect rather than a provider/search regression. The 60 s cloud job `4278786c-a243-4e0e-a37e-d3e2226b8434` failed with `Visual review request exceeded bounded input budget`. At 18 beat-aligned segments the review payload contains one global prompt, one segment header per beat, and two input items per candidate (label + image). Requiring at least two candidates per segment therefore produces at least 91 items, which cannot fit the existing 80-item cap. The 30 s Fleming job `81394bf5-b345-4565-82a4-13d4cc16dd76` exposed the companion quality problem: one whole-video request allowed only three candidate images per beat and the reviewer found just one exact match for a two-shot beat.

The correction batches review, not search and not acceptance thresholds. `Prepare Multimodal Visual Review` now creates deterministic batches of at most six beats. With six beats it can show six still candidates per beat within 79 input items; smaller batches expose more alternatives. `Require Multimodal Visual Selection` reconstructs all batch responses, keeps only candidate IDs actually shown in the corresponding batch, requires at least the planned shot count per beat, and can retain up to four reviewed alternatives for the global no-repeat beam assignment. Invalid/short model output still fails closed; no local or topic-anchor fallback returns.

Focused Node regression PASSes the 18-beat/3-batch boundary, per-batch <=80 item cap, complete chronological reconstruction, exact-model approval-only contract, insufficient-approval rejection, and multi-batch output ordering. Full suite and production proof are still required before acceptance.

## 2026-09-05 — bounded exact-query recovery and measured TTS bracket controller
