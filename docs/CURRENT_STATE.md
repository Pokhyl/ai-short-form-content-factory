...
- the first WF02 export attempt using the n8n CLI has now reached `Successfully exported 1 workflow.` after `n8n export:workflow --id=TJfA4ZYUEKSTad6k`. The surrounding shell then exited before the next file-existence validation, so the generated container path, file size, SHA-256, host copy, repository copy, structural export verification, secret scan, commit, and push are not yet confirmed. The export command itself must not be rerun until the already-created `/tmp/WF02-plan-script-and-scenes-*.json` candidate(s) inside the production n8n container are inspected;
...
Inspect the already-created `/tmp/WF02-plan-script-and-scenes-*.json` candidate(s) inside the production n8n container without rerunning `export:workflow`. Identify the export produced by the successful CLI run, verify it is valid JSON for workflow `TJfA4ZYUEKSTad6k`, verify the seven-node WF02 topology and updated M4 planning contract, verify credential objects are reference-only and runtime secrets are absent, then copy that verified export to `n8n/workflows/WF02-plan-script-and-scenes.json`, commit it, and push it on `feat/m4-script-scene-planning`. Do not add PostgreSQL scene persistence or job-state mutation until this export is safely committed. Manual script/visual quality acceptance for a meaningful real topic remains required before M4 can be considered complete.
