...
- a fresh M4 manual-quality test job has been created through the normal WF01 intake boundary for a meaningful real topic. The request used topic `Jak działa Kanał Panamski i jego system śluz`, language `pl`, and duration `30`; WF01 returned new `job_id = 80638242-ebee-4848-a1a4-9090446a95a8`. WF02 has not yet been run for this job, so its planning result and database persistence are not yet accepted;
...
Run the complete eight-node WF02 once for quality-test job `80638242-ebee-4848-a1a4-9090446a95a8`. Require exactly 8 scenes for the 30-second Polish job. Review the resulting Polish narration and visual descriptions for readability/coherence, verify `visual_subject_type` classification quality and English visual-query relevance, and then verify PostgreSQL persisted exactly 8 scenes atomically with the job in `processing/script`. Do not start Voiceover Generation or M5 until this quality acceptance passes.
