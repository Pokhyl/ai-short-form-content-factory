...
Perform a read-only inspection of the saved live WF02 `Generate Structured Plan` `jsonBody` and `Validate Structured Plan` `jsCode` for workflow `TJfA4ZYUEKSTad6k`. Determine which saved node configuration still lacks the production M4 contract, because the first 15-scene real run omitted `visual_subject_type` from every scene and the validator accepted that invalid result. Correct only the mismatched node configuration(s), then rerun one real Gemini request and require the validator to return exactly 15 sequential scenes with `visual_subject_type = factual|generic`, non-empty target-language narration/visual descriptions, and non-empty unique English visual queries no longer than 100 characters. Do not add PostgreSQL scene persistence or job-state mutation until this contract passes. After the live contract passes, export production WF02 into the repository before adding persistence.
