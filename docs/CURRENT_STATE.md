...
Perform one read-only PostgreSQL inspection of all eight persisted scenes for second quality-test job `ac32c553-dacd-4a21-bd7e-af3cd4941a82`. Review the Polish narration and visual descriptions for natural grammar/readability and scene-to-scene coherence, verify the factual claims are accurate rather than guessed, confirm factual/generic classifications are sensible and English visual queries are relevant. Also confirm the job remains `processing/script` with exactly 8 scenes. Do not rerun this job. M4 remains blocked and M5 must not start until this manual quality review passes.
