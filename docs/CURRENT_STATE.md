...
Create a fresh third meaningful-topic quality-test job through the normal WF01 intake boundary using topic `Jak działa Kanał Panamski i jego system śluz`, language `pl`, and duration 30 seconds. Run the complete eight-node WF02 once for that new job, then perform one read-only PostgreSQL inspection of its exactly 8 persisted scenes. Manually review target-language grammar/readability, scene-to-scene coherence, factual accuracy, narration-to-visual alignment, factual/generic classifications, official names/spelling where applicable, and English visual-query relevance. Do not rerun either prior quality-test job. M4 remains blocked and M5 must not start until this fresh run passes manual quality acceptance and persistence verification.
