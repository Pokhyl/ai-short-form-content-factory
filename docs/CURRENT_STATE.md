...
- a read-only inspection of the live production WF02 identified exactly one matching workflow: ID `TJfA4ZYUEKSTad6k`, name `WF02 — Plan Script and Scenes`. Its current planning chain is `Require Eligible Job` -> `Build Planning Request` -> `Generate Structured Plan` -> `Validate Structured Plan`, with no node after validation. The current `Build Planning Request` still uses the older open-ended scene contract and has no duration-specific required scene count or `visual_subject_type`; the Gemini `responseJsonSchema` has only `minItems: 1`, no exact `maxItems`, and no `visual_subject_type`; `Validate Structured Plan` currently requires only a non-empty scene array and four fields (`scene_number`, `narration`, `visual_description`, `visual_query`) and does not yet enforce the production 4/8/12/15 scene count, `visual_subject_type`, query length, or case-insensitive query uniqueness. The inspection modified nothing;
