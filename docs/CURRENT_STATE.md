...
Perform a read-only inspection of the current application PostgreSQL schema/migrations and retained M3 job state required for M4 atomic persistence. Confirm the exact `jobs` and `scenes` columns, constraints, indexes, current job eligibility/state, and any existing database mechanism relevant to writing the validated scene plan. Do not add or execute scene persistence until that inspection is complete. After the contract is verified, implement the smallest WF02 persistence step that writes all validated scenes and the required job-state transition in one PostgreSQL transaction. Do not implement Voiceover Generation or start M5. Manual script/visual quality acceptance on a meaningful real topic remains required before M4 can be considered complete.
