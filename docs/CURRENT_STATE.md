...
- a fresh third meaningful-topic M4 quality-test job has been created through the normal WF01 intake boundary with topic `Jak działa Kanał Panamski i jego system śluz`, language `pl`, and duration 30 seconds: `job_id = 1ff6453a-177c-4dc4-9b16-3cac05b2243a`. It has not yet been run through WF02;
...
Run the complete eight-node WF02 exactly once for third quality-test job `1ff6453a-177c-4dc4-9b16-3cac05b2243a`. After it completes, perform one read-only PostgreSQL inspection of the persisted scenes and verify that the 30-second job has exactly 8 scenes. Manually review target-language grammar/readability, scene-to-scene coherence, factual accuracy, narration-to-visual alignment, factual/generic classifications, official names/spelling where applicable, and English visual-query relevance. Do not rerun either prior quality-test job. M4 remains blocked and M5 must not start until this fresh run passes manual quality acceptance and persistence verification.
