...
Run M4 manual quality acceptance on a meaningful real topic rather than the synthetic M3 marker. Create a fresh eligible job through the normal intake boundary, run the complete eight-node WF02 once, and review the resulting narration, scene coherence, factual/generic classifications, visual descriptions, and English visual queries for real content quality. Verify the expected duration-specific scene count is persisted atomically in PostgreSQL. Do not start Voiceover Generation or M5 until this M4 quality acceptance passes.
