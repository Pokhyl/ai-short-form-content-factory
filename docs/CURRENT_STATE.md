...
Update only the live WF02 `Build Planning Request` prompt so generated narration and visual descriptions must be natural and grammatically correct in the target language, and factual claims must be accurate rather than guessed. Explicitly tell the model not to invent mechanisms or technical details and to prefer a simpler accurate statement when uncertain. Do not change the JSON schema, validator, persistence SQL, topology, credentials, database schema, or job-state contract. After saving that prompt-only change, record the runtime checkpoint, create a fresh eligible quality-test job, and rerun the complete eight-node WF02. M4 remains blocked and M5 must not start until a meaningful-topic run passes manual language/coherence/factual-quality review and PostgreSQL persistence verification.
