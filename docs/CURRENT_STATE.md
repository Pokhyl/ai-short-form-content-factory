...
- the production v1 M4 scene contract is an explicit project decision: 15/30/45/60-second jobs require exactly 4/8/12/15 scenes, and each scene maps to one narration segment, one later voiceover file, and one rendered timeline segment;
...
Update only the saved live WF02 `Generate Structured Plan` and `Validate Structured Plan` nodes in workflow `TJfA4ZYUEKSTad6k` to the already-defined production M4 contract; `Build Planning Request` is already correct and must not be changed. The Gemini schema must require exactly the duration-derived scene count and include `visual_subject_type` as required `factual|generic`; the validator must require the exact 4/8/12/15 count, sequential numbering, five exact fields, valid `visual_subject_type`, non-empty strings, `visual_query` no longer than 100 characters, and case-insensitive query uniqueness. Then rerun one real 60-second Gemini request and require a validator pass with exactly 15 valid scenes. Do not add PostgreSQL scene persistence or job-state mutation until this contract passes. After the live contract passes, export production WF02 into the repository before adding persistence.
...
- no extra microservices without a current requirement;
