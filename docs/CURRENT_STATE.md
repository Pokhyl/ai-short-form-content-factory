...
Run the updated live WF02 `Generate Structured Plan` and `Validate Structured Plan` nodes once for the retained 60-second M3 job `ba081017-2345-4212-a1c4-cde6df8de574`. Confirm one real Gemini request completes with `STOP` and that the validator accepts exactly 15 sequential scenes with non-empty target-language `narration` and `visual_description`, `visual_subject_type` exactly `factual|generic`, and non-empty English `visual_query` values no longer than 100 characters and unique case-insensitively. Do not add PostgreSQL scene persistence or job-state mutation until this new contract passes. After the live contract passes, export the production WF02 workflow into the repository before adding persistence.
