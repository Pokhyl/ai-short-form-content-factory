...
const normalizeKilo=by['Normalize Kilo Response'];
assert.ok(normalizeKilo,'Kilo response normalizer missing');
const normalizeCode=String(normalizeKilo.parameters?.jsCode??'');
assert.ok(normalizeCode.includes('finish_reason'),'Kilo normalizer must inspect finish_reason');
const normalizeFn=new Function('$json','$',normalizeCode);
const fakeDollar=(name)=>({first:()=>({json:{kilo_request_body:{},gemini_request_body:{},provider_attempts:[]}})});
const truncated=normalizeFn({choices:[{finish_reason:'length',message:{content:'{\"partial\":true'}}],model:'stepfun/step-3.7-flash:free',id:'truncated'},fakeDollar)[0].json;
assert.equal(truncated.kilo_success,false,'finish_reason=length must fall through to Gemini');
assert.equal(truncated.text,'','truncated Kilo text must never be returned as usable output');
assert.match(String(truncated.provider_attempts?.[0]?.error??''),/finish_reason:length/,'truncated attempt must record finish reason');
const filtered=normalizeFn({choices:[{finish_reason:'content_filter',message:{content:'partial'}}]},fakeDollar)[0].json;
assert.equal(filtered.kilo_success,false,'content-filtered output must fall through to Gemini');
const complete=normalizeFn({choices:[{finish_reason:'stop',message:{content:'complete output'}}],model:'stepfun/step-3.7-flash:free',id:'complete'},fakeDollar)[0].json;
assert.equal(complete.kilo_success,true,'complete stop-terminated Kilo output should remain usable');
assert.equal(complete.text,'complete output');
