...
const response={provider_exhausted:false,model:'cohere/north-mini-code:free',provider:'kilo-storyboard',text:JSON.stringify({scenes:[
 {scene_id:'SC1',purpose:'hook',grounded_fact_ids:['S1'],narration_intent:'Show the real sky first',shots:[{shot_id:'SC1A',representation:'context_media',must_show:'blue sky',must_not_show:'night sky',search_query_en:'clear blue sky',communication_goal:'Viewer sees a blue sky',crop_policy:'portrait_required',diagram_spec:null}]},
 {scene_id:'SC2',purpose:'explain',grounded_fact_ids:['S1','S2'],narration_intent:'Explain the scattering process',shots:[{shot_id:'SC2A',representation:'diagram',must_show:'light scatters',must_not_show:'wrong mechanism',search_query_en:'',communication_goal:'Viewer understands the mechanism',crop_policy:'not_applicable',diagram_spec:{archetype:'flow',short_title:'Scattering',entities:[{id:'sun',label:'Sun',role:'source',shape:'circle'},{id:'air',label:'Air',role:'process',shape:'box'},{id:'blue',label:'Blue',role:'output',shape:'arrow'}],relations:[{from:'sun',to:'air',kind:'enters'},{from:'air',to:'blue',kind:'scatters'},{from:'blue',to:'viewer',kind:'reaches'}]}}]},
...
assert.deepEqual(data.storyboard_scenes.map(s=>s.scene_id),['S1','S2','S3']);
...
assert.equal(data.storyboard_scenes[0].scene_id,'S1');
assert.equal(data.storyboard_scenes[0].shots[0].shot_id,'S1A');
