...
for(const subject of ['clock','telescope']){
 ...
 assert.equal(xs.length,28);assert.equal(new Set(xs.map(x=>x.asset_identity)).size,28);assert.equal(out.length,2,'expanded short-job review inventory may span two bounded batches');
...
assert.equal(exposed(many).length,48);assert.equal(many.length,2);
