...
const model=by.get('Nemotron Free Model');
assert.equal(model?.parameters?.url,'https://api.kilo.ai/api/gateway/chat/completions');
assert.ok(Number(model?.parameters?.options?.timeout)<=45000,'Nemotron timeout must stay bounded at 45s');
assert.equal(wf.connections?.['Build Request']?.main?.[0]?.[0]?.node,'Nemotron Free Model');
assert.equal(wf.connections?.['Normalize Nemotron Response']?.main?.[0]?.[0]?.node,'Respond');
const normalize=by.get('Normalize Nemotron Response')?.parameters?.jsCode??'';
assert.match(normalize,/provider_exhausted:!success/);
assert.match(normalize,/kilo-nemotron/);
