...
for(let i=0;i<uk.segments.length;i++){
 ...
 assert.equal(s.planned_shot_count,s.duration_seconds>=3.2?2:1,'only longer segments should use a second still');
...
assert.ok(zipper.segments.every(s=>s.planned_shot_count===(s.duration_seconds>=3.2?2:1)));
