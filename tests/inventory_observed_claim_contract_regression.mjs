...
const candidateClaims=[
 {claim_number:1,claim_id:'C1',claim:'Fact about visible component alpha.',visual_target:'Visible component alpha.',evidence_ids:['S1']},
 {claim_number:2,claim_id:'C2',claim:'Fact about visible component beta.',visual_target:'Visible component beta.',evidence_ids:['S2']},
 {claim_number:3,claim_id:'C3',claim:'Fact about visible component gamma.',visual_target:'Visible component gamma.',evidence_ids:['S3']},
];
const claims=[
...
assert.equal(out.verified_claim_count,3,'all three grounded claims must have a two-photo reserve');
...
assert(out.verified_claims.every(x=>x.assets.length===2),'each accepted fact must reserve two reviewer-approved photos before narration');
assert.equal(new Set(out.verified_claims.flatMap(x=>x.assets.map(a=>`${a.provider}:${a.provider_asset_id}`))).size,6);
assert.deepEqual(out.verified_claims[0].evidence_ids,['S1']);
const beta=out.verified_claims.find(x=>x.source_claim_id==='C2');
assert(beta.assets.some(a=>a.metadata.origin_claim_id==='C1'),'an image retrieved for another hypothesis may support an existing grounded claim');
assert(beta.assets.some(a=>a.metadata.origin_claim_id==='C2'));
for(const change of [{supported_claim_id:'C999'},{supported_claim_id:''},{relevant:false}]){
 ...
 assert.throws(()=>run(bad),/two-photo claim inventory 2\/3/,'invalid global grounding cannot create a two-photo claim reserve');
}
...
assert.equal(run([...verdicts].reverse()).verified_claim_count,3,'flat IDs must preserve grounded bindings regardless of retrieval group or response order');
