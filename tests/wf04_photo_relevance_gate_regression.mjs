...
assert.match(n4.get('Prepare Multimodal Visual Review').parameters.jsCode, /reject lexical coincidences/i);
assert.match(n4.get('Prepare Multimodal Visual Review').parameters.jsCode, /HARD REJECT every title card, infographic, chart, graph, diagram/);
assert.match(n4.get('Prepare Multimodal Visual Review').parameters.jsCode, /prominent\/legible embedded text/);
assert.match(n4.get('Prepare Multimodal Visual Review').parameters.jsCode, /media_kind\?\?'photo'\)\.toLowerCase\(\)===\x27photo\x27/);
