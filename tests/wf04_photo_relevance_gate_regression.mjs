...
assert.ok(prepared.segments.every(segment=>segment.visual_review_candidates.length===3),'only candidates actually shown to the reviewer may remain eligible');
assert.match(n4.get('Prepare Multimodal Visual Review').parameters.jsCode,/required images/);
assert.match(n4.get('Prepare Multimodal Visual Review').parameters.jsCode, /final timeline requires/);
assert.match(n4.get('Prepare Multimodal Visual Review').parameters.jsCode, /never repeat an ID/i);
...
assert.throws(()=>review({text:JSON.stringify({topic_anchor_candidate_ids:['photo:good'],segments:[{segment_number:1,selected_candidate_ids:[]}]})},review$),/approved 0\/1 relevant images/);
...
const approvedTwo=review({text:JSON.stringify({segments:[{segment_number:1,selected_candidate_ids:['photo:good','photo:garbage'],reasons:{'photo:good':'visible exact subject','photo:garbage':'second visibly relevant fixture'}}]})},twoShot$)[0].json.ranked_candidates;
...
assert.throws(()=>review({text:'not-json'},review$),/approved 0\/1 relevant images/);
