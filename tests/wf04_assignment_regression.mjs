...
const H={A:'0'.repeat(64),B:'f'.repeat(64),C:'a'.repeat(64),D:'5'.repeat(64),E:'3'.repeat(64),F:'c'.repeat(64)};const c=(id,u)=>({candidate_id:id,media_kind:'photo',visual_hash:H[id],selection_utility:u,local_rank_score:u,local_siglip_rank:1});
const rows=[
 ...
 {segment_number:4,start_seconds:12,end_seconds:16,duration_seconds:4,planned_shot_count:1,ranked_candidates:[c('F',.99)]},
...
const out=run({all:()=>rows.map(json=>({json}))},()=>({first:()=>({json:{visual_segment_count:rows.length}})})).map(x=>x.json);assert.equal(out.length,6);assert.deepEqual(out.map(x=>x.shot_number),[1,2,3,4,5,6]);assert(out.every(x=>x.segment_shot_number===1));assert.equal(out[0].candidate_id,'A');assert.equal(out[2].candidate_id,'C');assert.equal(new Set(out.map(x=>x.candidate_id)).size,6);assert.equal(out[0].visual_quality_pre_render.all_assets_unique,true);assert.equal(out[0].visual_quality_pre_render.asset_reuse_count,0);assert(out.every(x=>x.selection_method==='multimodal_exact_beat_shot_beam_v4'));assert(out.every(x=>x.visual_quality_pre_render?.pass===true));
const impossible=[
...
];assert.throws(()=>run({all:()=>impossible.map(json=>({json}))},()=>({first:()=>({json:{visual_segment_count:impossible.length}})})),/No valid semantic visual shot assignment/);
