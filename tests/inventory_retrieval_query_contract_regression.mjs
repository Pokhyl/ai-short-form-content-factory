...
const roles=['hook','mechanism','detail','result'];const claims=Array.from({length:4},(_,i)=>({claim_id:`C${i+1}`,claim:i===1?'Turbine runner blades transfer force to the shaft.':i===2?'Governor linkage adjusts the response.':`Supported fact ${i+1}.`,evidence_ids:[i%2?'S2':'S1'],editorial_role:roles[i],visual_form:i===1?'diagram':'photo',visual_target:i===1?'Subject turbine runner blades':i===2?'Subject governor linkage':`Subject component ${i+1}`,search_query_en:`subject mechanism ${i+1}`,inventory_asset_ids:[`V${i+1}`]}));
const base={research_rows:[{id:'S1'},{id:'S2'}],candidate_claim_range:[4,6],canonical_subject:'Subject',available_visual_assets:Array.from({length:6},(_,i)=>({inventory_id:`V${i+1}`,preclaim_visual_form:i===1?'diagram':'photo',preclaim_visible_description:i===1?'Subject turbine runner blades diagram':i===2?'Subject governor linkage':`Subject component ${i+1}`,title:i===1?'Subject turbine runner blades diagram':i===2?'Subject governor linkage':`Subject component ${i+1}`,description:`Subject mechanism component ${i+1}`,categories:'Subject'}))};
