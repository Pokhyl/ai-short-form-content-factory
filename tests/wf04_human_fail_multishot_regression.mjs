...
for(let i=0;i<3;i++){
  ...
  units.push({unit_id:`U${i+1}`,claim_id:`C${i+1}`,narration:`Unit ${i+1}.`,evidence_ids:[`S${i+1}`],editorial_role:['hook','mechanism','result'][i],visual_form:'photo',asset_id:shot_assets[0].asset_id,asset_ids:shot_assets.map(a=>a.asset_id),visual_target:`Target ${i+1}`,shot_plan:shot_assets.map((a,p)=>({asset_id:a.asset_id,shot_intent:`Notice verified subject ${i+1}-${p+1}`}))});
  bundles.push({...shot_assets[0],claim_id:`C${i+1}`,visual_target:`Target ${i+1}`,shot_assets});
}
assert.equal(identities.size,6);
const job={job_id:'11111111-1111-4111-8111-111111111111',topic:'Fixture',fact_primary_title:'Fixture',story_package:{version:'inventory-first-story-v1',editorial_contract_version:'shot-intent-v1',units,assets:bundles},scenes};
...
assert.equal(new Set(out.map(x=>x.reserved_visual_hash)).size,6,'every shot must use a distinct pre-script preview cluster');assert.deepEqual(out.slice(0,2).map(x=>x.shot_intent),['Notice verified subject 1-1','Notice verified subject 1-2']);assert.deepEqual(out.slice(0,2).map(x=>x.editorial_role),['hook','hook']);
let previous=0;
