...
assert.match(build,/model_request:\{prompt:x\.storyboard_prompt,response_format:'json',response_schema:storyboard_response_schema\}/);
...
assert.match(freeze,/grounded_fact_ids:shot\.diagram_spec\.grounded_fact_ids/);
...
const payload={scenes:[
 ...
 {scene_id:'S2',purpose:'explain',grounded_fact_ids:['S1','S2','S3','S4'],narration_intent:'Explain that shorter blue wavelengths scatter more',shots:[diagram('S2A',['S2','S3'])]},
...
assert.deepEqual(out.storyboard_scenes[1].shots[0].diagram_spec.grounded_fact_ids,['S2','S3']);
