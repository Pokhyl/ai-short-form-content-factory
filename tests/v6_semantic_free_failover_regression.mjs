...
const gate=by.get('Use Semantic Fallback?');assert.ok(gate);assert.match(JSON.stringify(gate.parameters),/free_model_success/);assert.match(JSON.stringify(gate.parameters),/route === 'semantic'/);assert.match(JSON.stringify(gate.parameters),/route === 'resolution'/);
