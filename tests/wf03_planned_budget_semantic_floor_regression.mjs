...
assert.equal(planned.desired_word_target,32,'measured proportional target must remain reachable for the exact production class');
assert.equal(planned.planned_word_max,42);
assert.equal(planned.max_fit_passes,1);
assert.doesNotMatch(planned.duration_controller_mode,/semantic_floor$/);
