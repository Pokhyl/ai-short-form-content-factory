...
let checked=0;
for(const file of files){const raw=JSON.parse(fs.readFileSync(file,'utf8')),wf=Array.isArray(raw)?raw[0]:raw;for(const n of wf.nodes){const body=String(wf.nodes.find(x=>x.name==='Build '+n.name+' Request')?.parameters?.jsCode??n.parameters?.jsonBody??'');if(n.type==='n8n-nodes-base.httpRequest'&&String(n.parameters?.url??'').includes('v4-model-gateway')&&/input:\s*(?:\$json|item)\.input/u.test(body)){checked++;assert.match(body,/response_format:\s*'json'/,`${n.name} must request structured JSON`);assert.match(body,/response_schema/,`${n.name} must send a schema`);}}}
