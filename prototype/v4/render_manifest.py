...
def assemble_render_manifest(
    ...
    *,
    ...
    if not graphic_info.get("all_required_graphics_compiled", graphic_info.get("all_constructed_beats_compiled")):
        raise ValueError("required graphic beats are not fully compiled")
    ...
    for index, beat in enumerate(beats, 1):
        ...
        item = {"beat_id": beat_id,"start_seconds": float(beat["start_seconds"]),"end_seconds": float(beat["end_seconds"]),"layout": primary.get("layout"),"visible_subject": primary.get("visible_subject"),"overlays": copy.deepcopy(beat.get("overlays", []))}
        if source_class in {"exact", "annotated_exact"}:
            asset = beat.get("resolved_asset")
            if not isinstance(asset, dict):
                raise ValueError(f"{source_class} beat {beat_id} has no resolved_asset")
            asset_path = Path(str(asset.get("local_path") or ""))
            if not asset_path.is_file():
                raise ValueError(f"{source_class} beat {beat_id} asset file is missing: {asset_path}")
            expected_sha = str(asset.get("sha256") or "")
            actual_sha = sha256_file(asset_path)
            if actual_sha != expected_sha:
                raise ValueError(f"{source_class} beat {beat_id} asset hash mismatch")
            item["asset"] = {"path": str(asset_path),"sha256": actual_sha,"width": asset.get("width"),"height": asset.get("height"),"license": asset.get("license"),"page_url": asset.get("page_url"),"attribution_required": bool(asset.get("attribution_required")),"artist": asset.get("artist")}
            if source_class == "exact":
                ...
                if not isinstance(graphic, dict) or not graphic.get("elements"):
                    raise ValueError(f"annotated exact beat {beat_id} has no compiled_graphic")
                if (graphic.get("source") or {}).get("beat_id") != beat_id:
                    raise ValueError(f"annotated exact beat {beat_id} graphic provenance mismatch")
    ...
    for index, word in enumerate(words, 1):
        ...
        start = float(word.get("start") or 0); end = float(word.get("end") or 0); text = str(word.get("word") or "").strip()
    ...
    if profile: render_profile.update(profile)
    ...
    return {"manifest_version":"v4-render-manifest-1","duration_seconds":timeline_duration,"profile":render_profile,"audio":{"path":str(audio),"sha256":actual_audio_sha,"duration_seconds":whisper_duration},"captions":{"source":"actual_audio_whisper_words","language":whisper_payload.get("language"),"probability":whisper_payload.get("probability"),"items":captions},"visual_track":track,"provenance":{"script_sha256":provenance.get("script_sha256"),"audio_sha256":actual_audio_sha,"timing_source":provenance.get("timing_source"),"asset_resolver":"hash_verified_exact_media","graphic_compiler":(compiled_timeline.get("graphic_compilation") or {}).get("compiler")}}
