...
async function focalSpans(text,lang,lexicon,dictionary,subject){
  if(Array.isArray(subject.focalMemberSpans)&&subject.focalMemberSpans.length>=2){
    const spans=subject.focalMemberSpans.sort((a,b)=>a.start-b.start).map(c=>withSpan(c,text));
    return appendPossessionObject(spans,text,lang,lexicon,dictionary);
  }
  let list=subject.listText,offset=list?normalize(text).indexOf(normalize(list)):-1;
  if(!list&&(subject.mode==='previous_scene_anaphora'||LIST_HEAD.test(text))){const copula=LIST_COPULA.exec(text);if(copula){offset=copula.index+copula[0].length;list=text.slice(offset).replace(/[.!?]\s*$/u,'');}}
  if(list&&offset>=0){const candidates=await subjectCandidates(list,lang,lexicon,dictionary),unmatched=words(list).filter(w=>!candidates.some(c=>c.start<=w.start&&c.end>=w.end));if(unmatched.some(w=>!/^(?:and|та|і|и|oraz|i)$/iu.test(w.text)))fail('unresolved_focal_list',list);if(candidates.length>=2)return appendPossessionObject(namedListMembers(candidates,list).sort((a,b)=>a.start-b.start).map(c=>withSpan(c,text,offset)),text,lang,lexicon,dictionary);}
  if(list&&offset<0){const all=await subjectCandidates(text,lang,lexicon,dictionary),members=all.filter(c=>normalize(list).includes(normalize(c.surface)));if(members.length>=2)return appendPossessionObject(members.sort((a,b)=>a.start-b.start).map(c=>withSpan(c,text)),text,lang,lexicon,dictionary);}
